...
Usage:
  python -m scripts.broker.manage_brokers list
  python -m scripts.broker.manage_brokers test deriv
  python -m scripts.broker.manage_brokers test mt5
  python -m scripts.broker.manage_brokers add deriv
  python -m scripts.broker.manage_brokers add mt5
  python -m scripts.broker.manage_brokers remove <broker> <name>
"""
...
def cmd_list():
    ...
    if not accounts:
        ...
        print("\nAdd accounts with:")
        print("  python -m scripts.broker.manage_brokers add deriv")
        print("  python -m scripts.broker.manage_brokers add mt5")
        return
...
def cmd_test_deriv(name: str = "demo"):
    ...
    if not cred:
        ...
        print(f"\nAdd it with: python -m scripts.broker.manage_brokers add deriv")
        return False
...
def cmd_test_mt5(name: str = "default"):
    ...
    if not cred:
        ...
        print(f"\nAdd it with: python -m scripts.broker.manage_brokers add mt5")
        return False
...
def cmd_add_deriv():
    ...
    if success:
        ...
        print(f"\nTest it: python -m scripts.broker.manage_brokers test deriv {name}")
        return True
    else:
...
def cmd_add_mt5():
    ...
    if success:
        ...
        print(f"\nTest it: python -m scripts.broker.manage_brokers test mt5 {name}")
        return True
    else:
        print("❌ Failed to store credentials")
        return False
...
Examples:
  ...
  python -m scripts.broker.manage_brokers add deriv
  python -m scripts.broker.manage_brokers add mt5
  python -m scripts.broker.manage_brokers remove deriv demo
        """
    )
