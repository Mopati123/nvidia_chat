...
class LiveTradingSystem:
    ...
    def execute_trade(self, 
                     ...
                     token: Optional[Any] = None) -> Optional[Dict]:
        """
        ...
        if self.mode == "shadow" or not self.active_broker:
            return self._execute_shadow_trade(symbol, direction, volume, token)
    ...
    def _execute_shadow_trade(self, symbol, direction, volume, token) -> Dict:
        ...
        execution = self.shadow.execute_shadow(
            symbol, ohlcv, direction, "auto", token=token, require_token=True
        )
